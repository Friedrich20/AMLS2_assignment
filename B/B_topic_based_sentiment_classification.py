...
model_path = os.path.join(home_dir, 'Codes', 'B', 'B_lstm_model.h5')
...
class B:
    ...
    def preprocess_data(self):
        ...
        df = self.df.drop(columns=[0, 4])  # drop 0:id and 4:NaN
        df = df.rename(columns={1: "topic", 2: "label", 3: "tweet"})
    ...
    def build_model_lstm(self):
        ...
        # Output Layer (2 classes for sentiments)
    ...
    def plot_confusion_matrix(self, y_test, y_pred, labels, normalize=None):
        ...
        plt.savefig('b_confusion matrix')
        # plt.show()
        return plt
